    ...
    <div class="bill-info-container">
      ...
      <div class="bill-info-area">
        ...
        <div class="submit-bill">
          ...
          <div>总价：<span>{{(billInfo.sysSession.sessionPrice * billSeats.length * numb).toFixed(1)}}</span></div>
  ...
  methods: {
    ...
    async getAllDiscount() {
      try {
        ...
        console.log(resp);
        _this.discounts = resp.data.data;
    ...
    async payForBill() {
      ...
      message: `
        <div style="text-align:center;">
          <p>请选择支付方式：</p>
          <p><input type="radio" name="payMethod" value="cash" id="cash"><label for="cash">现金支付</label></p>
          <p><input type="radio" name="payMethod" value="wechat" id="wechat"><label for="wechat">微信支付</label></p>
          <p><input type="radio" name="payMethod" value="alipay" id="alipay"><label for="alipay">支付宝支付</label></p>
        </div>`
      }).then(async () =>{
        // 用户点击确定，执行支付逻辑
        // 获取用户选择的支付方式
        ...
        switch (payMethod) {
          case 'cash':
            this.billInfo.payment = "现金支付"
            break;
          case 'wechat':
            this.billInfo.payment = "微信支付"
            break;
          case 'alipay':
            this.billInfo.payment = "支付宝支付"
            break;
          default:
            break;
        }
        this.billInfo.billMoney =  this.billInfo.sysSession.sessionPrice * this.billSeats.length * this.numb ;
        //更新订单状态
    ...
    addDis(props, id) {
      console.log(props.target.checked, id);
      if(props.target.checked){ // 勾选
        this.numb *= this.discounts[id].disOff;
      }
      else{      //取消
        this.numb /= this.discounts[id].disOff;
      }
